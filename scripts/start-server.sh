...
gunicorn agent.wsgi --bind 0.0.0.0:8000 --workers $WORKER_COUNT --timeout $WORKER_TIMEOUT --max-requests $WORKER_MAX_REQUEST --max-requests-jitter $WORKER_MAX_REQUEST_JITTER &
nginx -g "daemon off;"
