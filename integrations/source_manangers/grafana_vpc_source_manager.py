...
from integrations.source_api_processors.vpc_api_processor import VpcApiProcessor
...
class GrafanaVpcSourceManager(SourceManager):
    ...
    def execute_promql_metric_execution(self, time_range: TimeRange, grafana_task: Grafana,
        ...
        try:
            if not vpc_connector:
                raise Exception("Task execution Failed:: No VPC source found")
            ...
            print(
                ...
                "-> {}, End_Time -> {}, Period -> {}".format("Grafana", vpc_connector.account_id.value, datasource_uid,
                                                             promql_metric_query, start_time, end_time, period),
                flush=True)
